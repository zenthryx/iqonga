-- ==========================================
-- Lead Scoring Rules Migration (FIXED)
...
-- ==========================================
-- 4. INSERT DEFAULT SCORING RULE (OPTIONAL)
-- ==========================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM users WHERE id = 1) THEN
        ...
        RAISE NOTICE 'Default scoring rule created for user_id = 1';
    ELSE
        RAISE NOTICE 'User_id = 1 does not exist, skipping default rule creation';
    END IF;
END $$;

-- ==========================================
-- 5. HELPER FUNCTION: Get lead score breakdown
-- ==========================================
...
SELECT 
    ...
    (SELECT COUNT(*) FROM lead_scoring_rules) as rules_created,
    NOW() as completed_at;
