-- ==========================================
-- Sales Analytics Views Migration
...
DROP MATERIALIZED VIEW IF EXISTS sales_analytics_summary;
...
-- ==========================================
-- 4. SALES VELOCITY VIEW (Regular View)
-- ==========================================
...
SELECT 
    ...
    AVG(EXTRACT(DAY FROM (d.close_date - l.created_at))) as avg_days_to_close,
    MIN(EXTRACT(DAY FROM (d.close_date - l.created_at))) as fastest_close,
    MAX(EXTRACT(DAY FROM (d.close_date - l.created_at))) as slowest_close,
    AVG(d.amount) as avg_deal_size,
    SUM(d.amount) as total_revenue
FROM deals d
JOIN leads l ON d.lead_id = l.id
WHERE d.status = 'closed_won' AND d.close_date IS NOT NULL
GROUP BY d.user_id;
